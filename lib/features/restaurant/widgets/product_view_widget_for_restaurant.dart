...
import 'package:stackfood_multivendor/features/restaurant/widgets/product_widget_for_restaurant.dart';
...
class ProductViewWidgetForRestaurant extends StatelessWidget {
  ...
  const ProductViewWidgetForRestaurant({super.key, required this.restaurants, required this.products, this.isScrollable = false,
      ...
      !isNull ? length > 0 ? GridView.builder(
        ...
        itemBuilder: (context, index) {
          return showTheme1Restaurant ? RestaurantWidget(restaurant: restaurants![index], index: index, inStore: inRestaurantPage)
              : isWebRestaurant! ? WebRestaurantWidget(restaurant: restaurants![index]) : ProductWidgetForRestaurant(
        ...
        itemBuilder: (context, index) {
          return showTheme1Restaurant ? RestaurantShimmer(isEnable: isNull)
              : isWebRestaurant! ? const WebRestaurantShimmer() : ProductShimmer(isEnabled: isNull, isRestaurant: false, hasDivider: index != shimmerLength-1);
        },
      ),
